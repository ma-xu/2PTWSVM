function svcplot(X,Y,ker,w1,bias1,w2,bias2,c1,c2,d1,d2,aspect,mag,xaxis,yaxis,input)
...
%  Author: Steve Gunn (srg@ecs.soton.ac.uk)
    ...
    epsilon = 1e-5;  
    if (nargin < 16) input = zeros(1,size(X,2));, end
    if (nargin < 15) yaxis = 2;, end
    if (nargin < 14) xaxis = 1;, end
    if (nargin < 13) mag = 1;, end
    if (nargin < 12) aspect = 0;, end
